...
interface Text3DFlipProps {
  ...
  /** When true, run the 3D flip once after mount (useful when remounting on phrase change). */
  playOnMount?: boolean
  /** When false, skip hover-triggered flips (autoplay cycles). Default true. */
  interactive?: boolean
  /**
   * After the flip settles, snap faces back to identity so hover can replay.
   * Disable for remounting phrase cycles — the snap is a visible pop.
   */
  resetAfterFlip?: boolean
  /** Soft return duration when resetAfterFlip is true. 0 = instant (legacy). */
...
const Text3DFlip = ({
  ...
  return (
    <ElementTag
      ...
      onMouseEnter={interactive ? runFlip : undefined}
      ref={scope}
      {...props}
    >
