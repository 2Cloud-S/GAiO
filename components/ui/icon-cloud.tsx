      ...
      if (offCtx) {
        if (images) {
          ...
          img.onload = () => {
            ...
            // Soft disc so brand marks stay crisp on dark panels
            offCtx.beginPath()
            ...
            // Inset draw so glyphs aren't clipped at the rim
            offCtx.drawImage(img, 7, 7, 34, 34)
    ...
    if (canvas && ctx) {
      const animate = () => {
        ...
        iconPositions.forEach((icon, index) => {
          ...
          const scale = (rotatedZ + 240) / 360
          const opacity = Math.max(0.2, Math.min(1, (rotatedZ + 180) / 240))
